...
public class AutoPlaceSofa : MonoBehaviour, IInputClickHandler
{
    #region Singleton
    /// <summary>
    /// Singleton instance
    /// </summary>
    public static AutoPlaceSofa Instance { get; private set; }
    #endregion
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogError("More than 1 Auto Place Sofa instance detected! Deleting this new one.");
            Destroy(this);
            return;
        }
        #endregion
    ...
    private void GenerateItemsInWorld(List<Sofa> sofaList, List<GameObject> surfaces)
    {
        ...
        foreach (Sofa thissofa in sofaList)
        {
            /*
             * Placing the capsule
             */
            // Try to find a plane which we can place this poster by comparing their collider sizes
            int index = FindNearestPlane(surfaces, thissofa.GetComponent<Collider>().bounds.size, UsedPlanes);
            ...
            if (index >= 0)
            {
                ...
                thissofa.transform.position = surface.transform.position + (plane.SurfaceNormal * 0.2f);
