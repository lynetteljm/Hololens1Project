...
public class autoplacementsofa : MonoBehaviour, IInputClickHandler
{
    #region Singleton
    /// <summary>
    /// Singleton instance
    /// </summary>
    public static autoplacementsofa Instance { get; private set; }
    #endregion

    /*
     * Editor Exposed Variables
     */
    [Tooltip("The list of sofas to use.")]
    [SerializeField]
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogError("More than 1 autoplacementsofa instance detected! Deleting this new one.");
            Destroy(this);
            return;
        }
        #endregion
    ...
    private void OnPlanesCreated(object source, EventArgs args)
    {
        ...
        GenerateItemsInWorld(sofas, floorPlanes);
    }

    /// <summary>
    /// Creates and positions a collection of gameobjects on SurfacePlanes in the environment.
    /// </summary>
    /// <param name="sofaList">Collection of prefab GameObjects that have the Placeable component.</param>
    /// <param name="surfaces">Collection of SurfacePlane objects in the world.</param>
    private void GenerateItemsInWorld(List<Sofa> sofaList, List<GameObject> surfaces)
    {
        ...
        foreach (Sofa sofa in sofaList)
        {
            /*
             * Placing the capsule
             */
            // Try to find a plane which we can place this poster by comparing their collider sizes
            int index = FindNearestPlane(surfaces, sofa.GetComponent<Collider>().bounds.size, UsedPlanes);
            ...
            if (index >= 0)
            {
                ...
                sofa.transform.position = surface.transform.position + (plane.SurfaceNormal * 0.2f);
