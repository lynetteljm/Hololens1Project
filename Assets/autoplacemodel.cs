...
public class autoplacemodel : MonoBehaviour, IInputClickHandler
{
    #region Singleton
    /// <summary>
    /// Singleton instance
    /// </summary>
    public static autoplacemodel Instance { get; private set; }
    #endregion

    /*
    * Editor Exposed Variables
    */
    [Tooltip("The list of models to use.")]
    [SerializeField]
    private List<ModelLibrary> models;
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogError("More than 1 Auto Place Models instance detected! Deleting this new one.");
            Destroy(this);
            return;
        }
        #endregion
    ...
    private void OnPlanesCreated(object source, EventArgs args)
    {
        ...
        List<GameObject> tablePlanes = new List<GameObject>();
        ...
        tablePlanes = SurfaceMeshesToPlanes.Instance.GetActivePlanes(PlaneTypes.Table);
        ...
        GenerateItemsInWorld(models, tablePlanes);
    }

    /// <summary>
    /// Creates and positions a collection of gameobjects on SurfacePlanes in the environment.
    /// </summary>
    /// <param name="modelList">Collection of prefab GameObjects that have the Placeable component.</param>
    /// <param name="surfaces">Collection of SurfacePlane objects in the world.</param>
    private void GenerateItemsInWorld(List<ModelLibrary> modelList, List<GameObject> surfaces)
    {
        ...
        foreach (ModelLibrary model in modelList)
        {
            /*
             * Placing the Poster
             */
            // Try to find a plane which we can place this poster by comparing their collider sizes
            int index = FindNearestPlane(surfaces, model.GetComponent<Collider>().bounds.size, UsedPlanes);
            ...
            if (index >= 0)
            {
                ...
                model.transform.position = surface.transform.position + (plane.SurfaceNormal * 0.2f);
        ...
        {
           NetworkPlayer.Instance.CmdUpdateModelTransform(ModelLibrary.Instance.transform.localPosition, ModelLibrary.Instance.transform.localRotation);
        }
    }

    private int FindNearestPlane(List<GameObject> planes, Vector3 modelSize, List<int> usedPlanes)
    {
        ...
        for (int i = 0; i < planes.Count; i++)
        {
            ...
            if (modelSize.x > collider.bounds.size.x || modelSize.z > collider.bounds.size.z)
            {
