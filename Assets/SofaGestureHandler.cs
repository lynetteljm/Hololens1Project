...
public class SofaGestureHandler : MonoBehaviour, IManipulationHandler, ISpeechHandler
{
    /*
    * Serializable
    */
    ...
    [Tooltip("The multiplier for the rescaling gesture")]
    [SerializeField]
    private float rescaleSensitivity = 100.0f;
    [Header("Configuration")]
    [Tooltip("The reference to the Sofa Game Object")]
    [SerializeField]
    ...
    private bool isRescaleMode = false;
    ...
    public void OnManipulationStarted(ManipulationEventData eventData)
    {
        ...
        if (isRotationMode == false && isRescaleMode == false && isMoveMode == true)
        {
    ...
    public void OnManipulationUpdated(ManipulationEventData eventData)
    {
        ...
        else if (isMoveMode)    // Only do movement stuff if we intend to move
        {
        ...
        else if (isRescaleMode)
        {
            Sofa.transform.localScale *= 1.0f - (eventData.CumulativeDelta.z * rescaleSensitivity * Time.deltaTime);
        }
        if (NetworkPlayer.Instance != null)
        {
            ...
            NetworkPlayer.Instance.CmdUpdateSofaTransform(transform.localPosition, transform.localRotation, transform.localScale);
        }
    ...
    public void OnManipulationCompleted(ManipulationEventData eventData)
    {
        ...
        /************************************************************/
        // Call NetworkPlayer's "CmdUpdateModelTransform()" and pass it the local position and rotation
        //  NetworkPlayer.Instance.CmdUpdateCapsuleTransform(transform.localPosition, transform.localRotation, transform.localScale);
        //  }
    ...
    public void OnSpeechKeywordRecognized(SpeechEventData eventData)
    {
        ...
        if (eventData.RecognizedText == "rotating")
        {
            isRotationMode = true;
            isRescaleMode = false;
            isMoveMode = false;
        ...
        else if (eventData.RecognizedText == "moving")
        {
        ...
        else if (eventData.RecognizedText == "rescaling")
        {
            isRescaleMode = true;
            isRotationMode = false;
            isMoveMode = false;
